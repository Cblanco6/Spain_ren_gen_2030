...
function dispatch_electricity_market(hourly_data::DataFrame, fixed_data::DataFrame, interconnectors_delta::Vector; loss_factor::Float64 = 0.0, years_solving::Float64 = 0.230137)
    ...
    ph_nat_in = hourly_data.eff_75_Average * 0.8
    ...
    # # Import costs
    # @constraint(model, [t=1:T],
    #     import_costs[t] == (imports[t,1] - hourly_data.a_imp_FRA[t])^2/(2 * hourly_data.b_imp_FRA[t])
    #                      + (imports[t,2] - hourly_data.a_imp_POR[t])^2/(2 * hourly_data.b_imp_POR[t]) 
    #                      + (imports[t,3] - hourly_data.a_imp_MOR[t])^2/(2 * hourly_data.b_imp_MOR[t]));     

    # # Export revenues
    # @constraint(model, [t=1:T],
    #     export_revenues[t] == - (hourly_data.a_exp_FRA[t] - exports[t,1])^2/(2 * hourly_data.b_exp_FRA[t])
    #                         - (hourly_data.a_exp_POR[t]-exports[t,2])^2/(2 * hourly_data.b_exp_POR[t]) 
    #                         - (hourly_data.a_exp_MOR[t]-exports[t,3])^2/(2 * hourly_data.b_exp_MOR[t]));
    ...
    # According to our historical data, we observe a -20% output for conventional and run-of-river hydro
    # This is not the case for pumped hydro, but since we have disaggregated data only for 2023 onward, we estimate the same effect on natural inflows
    ...
    @constraint(model, [w in 1:n_bundles], sum(quantity[t, 9] for t in bundles[w]) <= 0.8 * hydro_weekly_totals[w]);
    ...
    @constraint(model, [t in high_prod_months], 0.8 * 0.30 * hourly_data.run_of_river_hydro_cap_mw[t] <= quantity[t,10] <= 0.8 * 0.50 * hourly_data.run_of_river_hydro_cap_mw[t]);
    @constraint(model, [t in med_high_prod_months], 0.8 * 0.20 * hourly_data.run_of_river_hydro_cap_mw[t] <= quantity[t,10] <= 0.8 * 0.40 * hourly_data.run_of_river_hydro_cap_mw[t]);
    @constraint(model, [t in med_low_prod_months], 0.8 * 0.15 * hourly_data.run_of_river_hydro_cap_mw[t] <= quantity[t,10] <= 0.8 * 0.30 * hourly_data.run_of_river_hydro_cap_mw[t]);
    @constraint(model, [t in low_prod_months], 0.8 * 0.10 * hourly_data.run_of_river_hydro_cap_mw[t] <= quantity[t,10] <= 0.8 * 0.15 * hourly_data.run_of_river_hydro_cap_mw[t]);
    @constraint(model, [t=2:T], quantity[t,10] - quantity[t-1,10] >= -0.2 * hourly_data.run_of_river_hydro_cap_mw[t]);
    @constraint(model, [t=2:T], quantity[t,10] - quantity[t-1,10] <= +0.2 * hourly_data.run_of_river_hydro_cap_mw[t]);

    # Pumped hydro
    @constraint(model, ph_stock[1] == 0.25 * storage_cap_ph); # Initial stock of pumped hydro is 25% of the capacity – lower due to drought
...
# Effects of climate change on demand (monthly adjusted)
temp_effects_demand = CSV.read(string(dirpath,"Data/scaled_temperature_effects.csv"), DataFrame)

# Define as numerical vectors the columns cc_delta_residential_demand_mwh, cc_delta_commercial_demand_mwh, cc_delta_industrial_demand_mwh
residential_demand_cc_effects = temp_effects_demand.cc_delta_residential_demand
commercial_demand_cc_effects = temp_effects_demand.cc_delta_commercial_demand
industrial_demand_cc_effects = temp_effects_demand.cc_delta_industrial_demand
...
for i in 201:num_iterations
    ...
    for var in variables_to_draw
        ...
        # Special handling for demand to incorporate temperature effects
        if endswith(var, "demand_mwh")
            base_delta = delta_draws[var]
            
            # Apply both the future growth delta and the climate change effects
            for i in 1:nrow(new_data)
                month_idx = new_data.month[i]
                year_idx = new_data.year[i] == 2023 ? 0 : 12  # Offset for accessing the right month in the effects vector
                effect_idx = month_idx + year_idx
                
                # Apply the appropriate climate effect based on the variable type
                if var == "residential_demand_mwh"
                    climate_effect = residential_demand_cc_effects[effect_idx]
                elseif var == "commercial_demand_mwh"
                    climate_effect = commercial_demand_cc_effects[effect_idx]
                elseif var == "industrial_demand_mwh"
                    climate_effect = industrial_demand_cc_effects[effect_idx]
                else
                    climate_effect = 0.0
                end
                
                # Apply both effects: future growth delta and climate change effect
                new_data[i, var] *= (1 + base_delta) * (1 + climate_effect)
            end
        else
            # For non-demand variables, nor the interconnectors, just apply the regular delta
            if !startswith(var, "imp_") && !startswith(var, "exp_")
    ...
    # Adjust capacity factors (only solar pv) with monthly variations
    solar_pv_cc_effects = -0.06318 #(-0.39%/◦C * 0.27◦C / decade * 0.6 decades) – Canadian Solar estimates
    for i in 1:nrow(new_data)
        month = new_data.month[i]
        
        # Apply different coefficients by month - adjusted to ensure weighted avg = 1
        if month in [12, 1, 2]  # Winter - minimal heat impact
            month_coef = 0.1  # Very minimal effect
        elseif month in [3, 4, 10, 11]  # Spring/Fall - moderate heat
            month_coef = 0.4  # 40% of base effect
        elseif month in [5, 9]  # Late Spring/Early Fall - higher heat
            month_coef = 1.2  # Slightly enhanced effect
        else  # Summer months (6, 7, 8) - maximum heat impact
            month_coef = 2.5  # Maximum effect
        end
        
        # Apply the adjusted effect
        new_data[i, :solar_pv_cap_factor] *= (1 + solar_pv_cc_effects * month_coef)
    end
    ...
    results = dispatch_electricity_market(new_data, fixed_data, interconnectors_delta, loss_factor = 0.02)
...
combined_results_path = string(dirpath, "Results/ClimateChange/combined_results.csv")
...
hourly_results_path = string(dirpath, "Results/ClimateChange/hourly_results.csv")
...
monthly_results_path = string(dirpath, "Results/ClimateChange/monthly_results.csv")
