...
function dispatch_electricity_market(hourly_data::DataFrame, fixed_data::DataFrame, interconnectors_delta::Vector; loss_factor::Float64 = 0.0, years_solving::Float64 = 0.230137)
    ...
    @constraint(model, [t=1:T], quantity[t,2] >= 0.02 * hourly_data.combined_cycle_cap_mw[t]);
    ...
    @constraint(model, [t=1:T], quantity[t,5] >= 0.05 * hourly_data.cogeneration_cap_mw[t]);
    @constraint(model, [t=1:T], quantity[t,5] <= 0.65 * hourly_data.cogeneration_cap_mw[t]);
    @constraint(model, [t=2:T], quantity[t,5] - quantity[t-1,5] >= -0.1 * hourly_data.cogeneration_cap_mw[t]);
    @constraint(model, [t=2:T], quantity[t,5] - quantity[t-1,5] <= +0.1 * hourly_data.cogeneration_cap_mw[t]);

    @constraint(model, [t=1:T], quantity[t,6] >= 0.15 * hourly_data.diesel_cap_mw[t]); # Minimum production in the Canary islands
    ...
    @constraint(model, [t=1:T], min_non_ren_gen[t] == 
                ...
                + 0.02 * hourly_data.combined_cycle_cap_mw[t] 
                + 0.05 * hourly_data.cogeneration_cap_mw[t]
                + 0.15 * hourly_data.diesel_cap_mw[t]
    ...
    @constraint(model, [t=1:T], quantity[t,15] <= 0.65 * hourly_data.other_renewable_cap_mw[t]);
    @constraint(model, [t=2:T], quantity[t,15] - quantity[t-1,15] >= -0.05 * hourly_data.other_renewable_cap_mw[t]);
    @constraint(model, [t=2:T], quantity[t,15] - quantity[t-1,15] <= +0.05 * hourly_data.other_renewable_cap_mw[t]); 

    @constraint(model, [t=1:T], quantity[t,16] <= 0.7 * hourly_data.renewable_waste_cap_mw[t]); 
    ...
    @constraint(model, [t=1:T], batt_out[t] <= 0.3 * hourly_data.batteries_cap_mw[t]);
    @constraint(model, [t=1:T], batt_in[t] <= 0.3 * hourly_data.batteries_cap_mw[t]);
...
for i in 201:num_iterations
    ...
    for var in variables_to_draw
        ...
        if var in ["residential_demand_mwh", "commercial_demand_mwh", "industrial_demand_mwh"]
            new_data[!, var] .*= (1 + 0.97 * delta_draws[var])

        # Only modify new_data if it's not an interconnector variable
        elseif !startswith(var, "imp_") && !startswith(var, "exp_")
            new_data[!, var] .*= (1 + delta_draws[var])
        end
    end

    for i in 1:nrow(new_data)
        # Apply smoothing reduction to residential demand with two peaks
        new_data[i, :residential_demand_mwh] = 
            new_data[i, :residential_demand_mwh] * (1 - 0.06 * exp(-((new_data.hour[i] - 11)^2) / 6) - 0.06 * exp(-((new_data.hour[i] - 19)^2) / 6))
    end
    ...
    delta_solar_pv_cap_factor = rand(Normal(0.05, 0.01))
    new_data.solar_pv_cap_factor .*= (1 + delta_solar_pv_cap_factor * (delta_draws["solar_pv_cap_mw"] / (1 + delta_draws["solar_pv_cap_mw"])))
    delta_solar_thermal_cap_factor = rand(Normal(0.05, 0.01))
    new_data.solar_thermal_cap_factor .*= (1 + delta_solar_thermal_cap_factor * (delta_draws["solar_thermal_cap_mw"] / (1 + delta_draws["solar_thermal_cap_mw"])))     
    delta_wind_cap_factor = rand(Normal(0.05, 0.01))
    new_data.wind_cap_factor .*= (1 + delta_wind_cap_factor * (delta_draws["wind_cap_mw"] / (1 + delta_draws["wind_cap_mw"])))
...
combined_results_path = string(dirpath, "Results/Optimistic/combined_results.csv")
...
hourly_results_path = string(dirpath, "Results/Optimistic/hourly_results.csv")
...
monthly_results_path = string(dirpath, "Results/Optimistic/monthly_results.csv")
